...
There is deliberately no matching constraint on `family_id`: a family may hold more than one tenancy at once (a home and a second property, for example). Preventing overlapping *historical* tenancies on a property is possible with a PostgreSQL exclusion constraint on a date range, but that is beyond the scope of this exercise; the partial unique index covers the case that matters operationally.
...
- A tenancy links one family to one property for a date range. At any point in time a property is occupied by at most one family (the partial unique index), while a family may occupy several properties, without hard-wiring `properties.family_id`: a family can move, a property can be re-let, and the history of both survives. That an occupying family has at least one tenant is an application rule, not something the schema can guarantee.
...
- `tenancies`: partial unique on `(property_id) WHERE end_date IS NULL`; plain `(property_id, end_date)` for "who lives here now" lookups.
...
- A family can occupy different properties over time, and may hold more than one current tenancy; a property has at most one current family.
